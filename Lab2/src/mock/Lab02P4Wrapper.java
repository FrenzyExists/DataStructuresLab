...
public class Lab02P4Wrapper {
    ...
    public static class LinkedBag<T> implements Bag<T> {
        ...
        public Bag<Integer> getFrequencyBag(){
            Bag<Integer> disBag = new LinkedBag<Integer>();
            HashMap<T, Integer> disMap = new HashMap<>();
            Node currrentNode = first;

            while (currrentNode != null) {
                if(!disMap.containsKey(currrentNode.getData())) {
                    disMap.put(currrentNode.getData(), 1);
                } else {
                    disMap.put(currrentNode.getData(), disMap.get(currrentNode.getData())+1);
                }

                currrentNode = currrentNode.getNext();
            }
            Iterator hmIterator = disMap.entrySet().iterator();
            while (hmIterator.hasNext()) {
                Map.Entry mapElement = (Map.Entry)hmIterator.next();
                disBag.add( (int)mapElement.getValue());
            }
            return disBag;
